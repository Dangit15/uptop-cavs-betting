        ...
        <div className="grid gap-10 md:grid-cols-[2fr_1.4fr] mt-10">
          <section className="bg-white rounded-3xl shadow-md p-8 space-y-8">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-600">
                {gameState.data
                  ? `Starts ${new Date(gameState.data.startTime).toLocaleString()}`
                  : "We’ll show the next Cavaliers matchup here as soon as odds are available."}
              </p>
              <p className="text-xs text-gray-400 mt-1">Last synced: when you opened this page</p>
            </div>

            {gameState.loading && <p className="text-sm text-gray-600">Loading next game…</p>}

            {gameState.error && (
              <p className="text-sm text-red-500">Error: {gameState.error}</p>
            )}
                ...
                {gameState.data ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <p className="text-sm text-gray-600">
                        Status:{" "}
                        <span className="font-semibold text-black">{gameState.data.status}</span>
                      </p>
                      <p className="text-sm text-gray-600">
                        Cavs spread:{" "}
                ...
                {status === "authenticated" && session?.user?.email === "admin@example.com" && (
                  <div className="border-t border-gray-100 pt-4 space-y-3">
                    ...
                    <button
                      ...
                      className="rounded-full border border-gray-300 text-gray-800 px-4 py-2 text-sm font-medium shadow-sm hover:border-gray-400 transition disabled:opacity-60"
                    >
